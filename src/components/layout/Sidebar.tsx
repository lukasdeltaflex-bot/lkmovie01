...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed top-0 left-0 h-full bg-surface border-r border-border-custom transition-all duration-300 flex flex-col z-95 shadow-xl dark:shadow-none 
      ...
      >
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto custom-scrollbar text-foreground">
          <div className={`text-[10px] font-black text-muted-custom uppercase tracking-widest mb-4 ml-2 transition-opacity duration-300 ${(sidebarCollapsed && !isMobileOpen) ? 'opacity-0' : 'opacity-100'}`}>Menu Principal</div>
          {navItems.map((item) => {
              ...
              <Link 
                ...
                className={`flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 overflow-hidden whitespace-nowrap group relative ${
                  isActive 
                    ...
                    : "text-muted-custom hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-foreground"
                }`}
        ...
        <div className="p-4 relative mt-auto border-t border-border-custom">
           {isMenuOpen && (!sidebarCollapsed || isMobileOpen) && (
              <div className="absolute bottom-[90px] left-4 right-4 bg-surface border border-border-custom rounded-3xl shadow-[0_30px_60px_-12px_rgba(0,0,0,0.5)] p-2 animate-in fade-in slide-in-from-bottom-2 duration-200 z-50 overflow-hidden">
                 ...
                 <div className="my-2 border-t border-border-custom opacity-50"></div>
                 <div className="px-4 py-2 flex items-center justify-between">
                   <span className="text-[10px] text-muted-custom font-bold uppercase tracking-wider">Interface</span>
           ...
           <div 
             ...
             className={`flex items-center gap-3 p-3 rounded-2xl hover:bg-gray-50 dark:bg-gray-800/10 dark:hover:bg-gray-800 transition-all cursor-pointer border border-border-custom ${(sidebarCollapsed && !isMobileOpen) ? "justify-center" : "shadow-sm"}`}
          >
            ...
            <div className={`transition-opacity duration-300 overflow-hidden flex-1 ${(sidebarCollapsed && !isMobileOpen) ? "opacity-0 w-0" : "opacity-100"}`}>
              <p className="text-sm font-black text-foreground truncate italic uppercase tracking-tighter"> {user?.email?.split('@')[0] || "Usuário"} </p>
              <p className="text-[10px] text-muted-custom font-bold truncate tracking-widest">{user?.email}</p>
            </div>
